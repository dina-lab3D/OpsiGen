...
class SimpleCNN(nn.Module):
    """suggestion for baseline to use on ESM embedding"""
    def __init__(self):
        super().__init__()
        ...
        self.fc1 = nn.Linear(128 * 12 * 250, 128)  # Adjust this based on the output size after pooling and conv layers
        self.fc2 = nn.Linear(128, 1)  # Number of output classes

    def forward(self, x):
        x = self.pool(F.relu(self.conv1(x)))
        x = self.pool(F.relu(self.conv2(x)))
        x = self.pool(F.relu(self.conv3(x)))
        x = x.view(-1, 128 * 12 * 250)  # Flatten the tensor
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return x